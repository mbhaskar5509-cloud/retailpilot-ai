...
export default function InventoryPage() {
  ...
  async function loadInventory() {
    ...
    try {
      // Create Supabase client only when the page is running in the browser.
      ...
      const movementData: Movement[] = (movementsResult.data ?? []).map(
        (movement) => {
          const rawMovement = movement as typeof movement & {
            products:
          ...
          const productRelation = Array.isArray(rawMovement.products)
            ? rawMovement.products[0] ?? null
            : rawMovement.products ?? null;

          const storeRelation = Array.isArray(rawMovement.stores)
            ? rawMovement.stores[0] ?? null
            : rawMovement.stores ?? null;
  ...
  async function addMovement(e: React.FormEvent) {
    ...
    try {
      // Create Supabase client at action time, not during build.
      const supabase = createClient();
      ...
      setMessage("Unable to save stock movement.");
    } finally {
      setSaving(false);
    }
  }
